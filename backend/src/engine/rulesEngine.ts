...
/**
 * Returns volume constraints adjusted for recovery status (sleep, stress) and
 * commitment level. Applied on top of the base trainingAge constraints.
 */
export function getAdjustedVolumeConstraints(
  trainingAge?: string,
  sleep?: string,
  stressEnergy?: string,
  commitment?: string
): VolumeConstraints {
  const base = getVolumeConstraints(trainingAge);
  let multiplier = 1.0;

  // Sleep penalty
  if (sleep === 'very_poor') multiplier -= 0.20;
  else if (sleep === 'poor') multiplier -= 0.12;

  // Stress/energy penalty
  if (stressEnergy === 'burnout') multiplier -= 0.20;
  else if (stressEnergy === 'high_stress') multiplier -= 0.10;

  // Commitment cap — low commitment should not receive max-volume programs
  if (commitment === '4') multiplier = Math.min(multiplier, 0.75);
  else if (commitment === '6') multiplier = Math.min(multiplier, 0.90);

  // Floor at 60% of base to maintain minimum effective dose
  multiplier = Math.max(multiplier, 0.60);

  return {
    maxTotalSets:    Math.round(base.maxTotalSets * multiplier),
    maxCompoundSets: Math.round(base.maxCompoundSets * multiplier),
    maxAccessorySets: Math.round(base.maxAccessorySets * multiplier),
  };
}

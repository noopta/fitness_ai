...
- For exercise swaps in their program: FIRST call read_schedule_week to get the exact stored exercise names, then swap_exercise_in_program(fromExerciseName, toExerciseName, reason?). The backend only swaps names that exist in the program — if it returns applied:false with candidates, pick the exact one and call again. Do NOT tell the user it's done unless applied:true.

  ...
  try {
    ...
    if (twilioClient && process.env.TWILIO_PHONE_NUMBER && process.env.NOTIFICATION_PHONE) {
      try {
        const message = await twilioClient.messages.create({
          body: `🚀 New Axiom Waitlist Signup!\nEmail: ${data.email}${data.name ? `\nName: ${data.name}` : ''}`,
        ...
        await sendEmail(
          ...
          `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              <h2 style="color: #2563eb;">🚀 New Axiom Waitlist Signup!</h2>
    ...
    if (useSendGrid || emailTransporter) {
      try {
        await sendEmail(
        data.email,
        '🎉 Congratulations! You\'re on the Axiom Waitlist',
        `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff;">
            ...
            <div style="padding: 40px 30px; background: white;">
              ...
              <p style="font-size: 16px; line-height: 1.8; color: #374151; margin-bottom: 20px;">
                You've signed up for the <strong>Axiom MVP</strong> and we're beyond happy to have you on board!
              ...
              <div style="text-align: center; margin: 35px 0;">
                <a href="https://axiomtraining.io/onboarding" style="display: inline-block; background: linear-gradient(135deg, #2563eb 0%, #3b82f6 100%); color: white; padding: 14px 32px; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 16px; box-shadow: 0 4px 6px rgba(37, 99, 235, 0.3);">
                  Try the MVP Now
                </a>
              </div>
              <p style="font-size: 16px; line-height: 1.6; color: #374151; margin-top: 30px;">
                Thanks for being part of this journey!<br><br>
                <strong style="color: #2563eb;">The Axiom Team</strong>
              </p>
            </div>
            <div style="background: #f9fafb; padding: 25px; text-align: center; border-radius: 0 0 12px 12px; border-top: 1px solid #e5e7eb;">
              <p style="color: #6b7280; font-size: 14px; margin: 0 0 10px 0;">
                <strong>Axiom</strong> - AI-Powered Lift Diagnostics
              </p>
              <p style="color: #9ca3af; font-size: 12px; margin: 0;">
                You're receiving this email because you signed up for the Axiom waitlist.
              </p>
            </div>
          </div>
        `

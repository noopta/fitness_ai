...
function estimateTDEE(params: { weightKg: number | null; heightCm: number | null; gender: string | null; activityLevel?: string | null }): number | null {
  ...
  // Activity multiplier — derived from coachProfile activityLevel
  ...
  const multiplier = activityMultipliers[params.activityLevel || ''] ?? 1.55;
  return Math.round(bmr * multiplier);
}

/**
 * Extracts body composition intent from the user's free-text primaryGoal.
 * Returns one of: fat_loss | muscle_gain | recomp | maintenance
 */
export function extractBodyCompositionGoal(primaryGoal: string | null | undefined): 'fat_loss' | 'muscle_gain' | 'recomp' | 'maintenance' {
  if (!primaryGoal) return 'maintenance';
  const text = primaryGoal.toLowerCase();
  if (/lose weight|fat loss|fat-loss|cut|leaner|lean out|deficit|drop.*lb|drop.*weight|lose.*lb|lose.*fat|shred|slim/.test(text)) return 'fat_loss';
  if (/bulk|gain muscle|gain mass|muscle mass|more muscle|build muscle|\bsize\b|bigger|mass|put on weight/.test(text)) return 'muscle_gain';
  if (/recomp|body recomposition|tone|toning|maintain.*muscle|lose fat.*gain|gain.*lose fat/.test(text)) return 'recomp';
  return 'maintenance';
}

export async function generateNutritionPlan(params: {
  goal: string;
  bodyCompositionGoal?: 'fat_loss' | 'muscle_gain' | 'recomp' | 'maintenance';
  weightKg: number | null;
  heightCm?: number | null;
  trainingAge: string | null;
  primaryLimiter: string | null;
  ...
  const tdee = estimateTDEE({ weightKg: params.weightKg, heightCm: params.heightCm || null, gender: params.gender || null, activityLevel: params.activityLevel });
  const tdeeNote = tdee
    ? `- Estimated TDEE (Mifflin-St Jeor × activity multiplier): ${tdee} kcal/day — use this as your maintenance baseline when setting caloric targets`
    : '';

  // Body composition directive takes priority over vague training style goal
  const bodyCompMap: Record<string, string> = {
    fat_loss:     'Fat loss — apply 300–500 kcal deficit below TDEE',
    muscle_gain:  'Muscle gain — apply 200–350 kcal surplus above TDEE',
    recomp:       'Body recomposition — calories at or near TDEE, high protein',
    maintenance:  'Maintenance / general performance',
  };
  const bodyCompDirective = params.bodyCompositionGoal
    ? `- Body composition goal: ${bodyCompMap[params.bodyCompositionGoal]}`
    : '';
...
ATHLETE PROFILE:
- Training style goal: ${params.goal || 'general strength'}
...
- The "Body composition goal" field above takes HIGHEST priority for setting calorie targets. If it says fat loss → apply 300–500 kcal deficit. If muscle gain → 200–350 kcal surplus. If recomp/maintenance → at TDEE.
...
export async function generateTrainingProgram(params: {
  ...
  if (params.coachProfile) {
    try {
      ...
      if (profile.pastAttempts) lines.push(`- Past attempts/struggles (what worked or didn't): ${profile.pastAttempts}`);
      if (profile.obstacleToConsistency) lines.push(`- Biggest obstacle to consistency: ${profile.obstacleToConsistency} — design the program to specifically address this`);
      if (profile.commitment) lines.push(`- Commitment level (4/6/8/10): ${profile.commitment} — adjust program intensity and volume accordingly; do not prescribe a demanding program to low-commitment users`);
      ...
      if (profile.activityLevel) lines.push(`- Daily activity outside training: ${profile.activityLevel}`);
      if (profile.lifestyle) lines.push(`- Lifestyle (typical day): ${profile.lifestyle}`);
      if (profile.recoveryPractices) lines.push(`- Recovery practices: ${profile.recoveryPractices} — factor into volume tolerance`);

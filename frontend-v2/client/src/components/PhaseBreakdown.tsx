          ...
          return (
            <div key={phase.id} className="space-y-2">
              <div className="flex items-center justify-between gap-4">
                ...
                {hasSignal ? (
                  <span className={`text-[11px] font-mono font-bold flex-shrink-0 ${isPrimary ? "text-red-500" : "text-orange-500"}`}>
                    {points} pts
                  </span>
                ) : (
                  <span className="text-[11px] text-muted-foreground flex-shrink-0">No signal</span>
                )}
              </div>
              ...
              {/* Phase description */}
              <p className="text-[11px] text-muted-foreground leading-relaxed pt-0.5">{phase.description}</p>
            </div>
          );
        })}
      </div>

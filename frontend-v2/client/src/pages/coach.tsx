...
export default function CoachPage() {
  ...
  // Parse savedProgram's embedded nutrition plan + duration
  let savedNutritionPlan: any = null;
  let savedProgramDurationWeeks: number | null = null;
  try {
    if (user?.savedProgram) {
      const prog = JSON.parse(user.savedProgram);
      savedNutritionPlan = prog?.nutritionPlan || null;
      savedProgramDurationWeeks = prog?.durationWeeks ?? null;
    }
  } catch { /* ignore */ }

  // Format programStartDate as YYYY-MM-DD string if present
  const programStartDate: string | null = user?.programStartDate
    ? new Date(user.programStartDate).toISOString().split('T')[0]
    : null;
